...
//		[Tooltip("Gradient colors to color lines. The left-most color represents the color for the first line(first order) and the right-most color for the last line(last order). ")]
//		public Gradient gradientColor;

//		[Header("Editor Options")]
//		public bool drawGizmo = true;
//		[Tooltip("When turned on, Line's gameobject will be automatically named according to its setup")]
//		public bool autoNameGameobject = true;
//		[Tooltip("When turned on and if there's a Text component assigned to a line, the line's order number is set to its text")]
//		public bool autoSetTextNumber = true;
//		[Tooltip("When turned on and using autoSetTextNumber, it will increment the number shown by one(Thus 0 will be displayed as 1)")]
//		public bool incrementTextNumber = true;
...
//		internal void OnRefreshLayout() { foreach (Line line in lines) line.SetPaths(); }
//		internal void OnStartRound() { foreach (Line line in lines) line.OnStartRound(); }
